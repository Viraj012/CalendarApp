...
public class CopyEventTest {
  ...
  public void testCopySingleEventToSameCalendar() {
    ...
    boolean result = manager.copyEvent("Test Event",
            startDateTime, "Source", targetDateTime);
    ...
    assertEquals("Copied event should have the same name",
            "Test Event", events.get(0).getSubject());
  }
  ...
  public void testCopySingleEventToDifferentCalendar() {
    ...
    boolean result = manager.copyEvent("Test Event", startDateTime,
  ...
  public void testCopyRecurringEvent() {
    ...
    LocalDateTime recurrStartTime = LocalDateTime
            .of(2023, 6, 5, 10, 0);
    LocalDateTime recurrEndTime = LocalDateTime
            .of(2023, 6, 5, 11, 0);
    ...
    boolean created = manager.getCurrentCalendar().createRecurringEvent(
        "Recurring Meeting", recurrStartTime, recurrEndTime,
            "MW", 3, null,
        true, "Recurring Description",
            "Recurring Location", true);
    ...
    boolean result = manager.copyEvent("Recurring Meeting", recurrStartTime,
            "Target", targetDateTime);
    ...
    assertEquals("There should be one event series in the target calendar",
            1, targetEvents.size());
  ...
  public void testCopyEventsOnDay() {
    ...
    manager.useCalendar("Target");
    List<Event> targetEvents = manager.getCurrentCalendar().getAllEvents();
    assertEquals("There should be three events in the target calendar",
            3, targetEvents.size());
  }
  ...
  public void testCopyEventsInRange() {
    ...
    manager.useCalendar("Target");
    List<Event> day1Events = manager.getCurrentCalendar().getEventsOn(LocalDateTime
            .of(2023, 5, 22, 0, 0));
    List<Event> day2Events = manager.getCurrentCalendar().getEventsOn(LocalDateTime
            .of(2023, 5, 23, 0, 0));
    List<Event> day3Events = manager.getCurrentCalendar().getEventsOn(LocalDateTime
            .of(2023, 5, 24, 0, 0));
  ...
  public void testCopyToNonExistentCalendar() {
    boolean result = manager.copyEvent("Test Event", startDateTime,
            "NonExistent", startDateTime.plusDays(1));
    assertFalse("Copying to non-existent calendar should fail", result);
  }

  @Test
  public void testCopyNonExistentEvent() {
    boolean result = manager.copyEvent("Non-Existent Event", startDateTime,
            "Target", startDateTime.plusDays(1));
  ...
  public void testAllDayEventCopy() {
    ...
    boolean created = manager.getCurrentCalendar().createAllDayEvent(
        "All-Day Event", allDayDate, true, "All-Day Description",
            "All-Day Location", true);
    assertTrue("Creating all-day event should succeed", created);

    // Copy to target calendar
    LocalDateTime targetDate = LocalDateTime
            .of(2023, 5, 27, 0, 0); // One week later
    boolean result = manager.copyEvent("All-Day Event", allDayDate,
            "Target", targetDate);
